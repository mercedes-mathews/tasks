...
export function findQuestion(
    ...
    const questionFromID = questions.find(
        (question: Question): boolean => question.id === id
    );
    return questionFromID === undefined ? null : questionFromID;
}
...
export function removeQuestion(questions: Question[], id: number): Question[] {
    const removedQuestionFromID = questions.filter(
        (question: Question): boolean => question.id !== id
    );
    return removedQuestionFromID;
}
...
export function sumPublishedPoints(questions: Question[]): number {
    ...
    const sumOfPublished = publishedQuestions.reduce(
        (currentSum: number, question: Question) =>
            currentSum + question.points,
        0
    );
...
export function sameType(questions: Question[]): boolean {
    const isTheSameType = questions.every(
...
export function changeQuestionTypeById(
    ...
    if (
        questionFromID !== undefined &&
        newQuestionType !== "multiple_choice_question"
    ) {
        questionFromID.type = newQuestionType;
        questionFromID.options = [];
    }
    return questionCollection;
}
...
export function duplicateQuestionInArray(
    ...
    const questionIndex = questionCollection.findIndex(
        (question: Question): boolean => question.id === targetId
    );
    questionCollection.splice(
        questionIndex + 1,
        0,
        duplicateQuestion(newId, questionCollection[questionIndex])
    );
    return questionCollection;
}
